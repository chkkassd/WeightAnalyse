 ...
 */
class LoginView: UIView,LoginStyle,HasBackgroundView,HasCancelButton,CheckInputting {
    ...
    func checkInput() -> Bool {
        if emailTextField.text == nil || emailTextField.text == ""{
            self.noticeInfo("Please input your email!", autoClear: true, autoClearTime: 2)
            return false
        } else if passwordTextField.text == nil || passwordTextField.text == ""{
            self.noticeInfo("Please input your password!", autoClear: true, autoClearTime: 2)
            return false
        } else {
            return true
        }
    ...
    func buttonPressed() {
        if checkInput() {
            self.delegate?.loginViewDidLogin(loginView: self, email: emailTextField.text!, password: passwordTextField.text!)
        }
    }
...
/**
 ...
 Now,it just fits in code,it' not useful to storyboard.
 */
class RegisterView: UIView,RegisterStyle,HasBackgroundView,HasCancelButton,CheckInputting {
    lazy var emailTextField: UITextField = {
    ...
    func checkInput() -> Bool {
        if emailTextField.text == nil || emailTextField.text == "" {
            ...
            self.noticeInfo("Please input your password!", autoClear: true, autoClearTime: 2)
            return false
        } else if nickNameTextfield.text == nil || nickNameTextfield.text == "" {
            self.noticeInfo("Please input your nickName!", autoClear: true, autoClearTime: 2)
            return false
        } else {
            return true
        }
    ...
    func buttonPressed() {
        if checkInput() {
            self.delegate?.registerViewDidRegister(registerView: self, email: emailTextField.text!, password: passwordTextField.text!, nickName: nickNameTextfield.text!)
        }
    }
